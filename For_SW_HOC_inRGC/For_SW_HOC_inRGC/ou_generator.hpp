...
#ifndef OU_GENERATOR_H
#define OU_GENERATOR_H
...
/* BeginDocumentation
Name: ou_generator - Device to generate Gaussian white ou current.
Description:
This device can be used to inject a Gaussian "white" ou current into a node.
...
mean      double - mean value of the ou current in pA
std       double - standard deviation of ou current in pA
dt        double - interval between changes in current in ms, default 1.0ms
std_mod   double - modulated standard deviation of ou current in pA
...
Remarks:
 ...
 - The effect of this ou current on a neuron DEPENDS ON DT. Consider
   the membrane potential fluctuations evoked when a ou current is
   ...
   for large t_j. In the white ou limit, dt -> 0, one has

     Sigma -> std / C_m * sqrt(dt * tau / 2).

   To obtain comparable results for different values of dt, you must
   adapt std.
 - As the ou generator provides a different current for each of its targets,
...
/**
 * Gaussian white ou generator.
 * Provide Gaussian "white" ou input current
 */
class ou_generator : public Node
{

public:
  ou_generator();
  ou_generator( const ou_generator& );
...
private:
  ...
  struct Parameters_
  {
    ...
    */
    ...
    void set( const DictionaryDatum&, const ou_generator& );
  };
  ...
  friend class RecordablesMap< ou_generator >;
  friend class UniversalDataLogger< ou_generator >;
  ...
  struct Buffers_
  {
    ...
    Buffers_( ou_generator& );
    Buffers_( const Buffers_&, ou_generator& );
    UniversalDataLogger< ou_generator > logger_;
  };
...
*/
  ...
  static RecordablesMap< ou_generator > recordablesMap_;
  Parameters_ P_;
  Variables_ V_;
  Buffers_ B_;
  State_ S_;
...
inline port
ou_generator::handles_test_event( DataLoggingRequest& dlr,
  rport receptor_type )
{
  if ( receptor_type != 0 )
  {
...
inline void
ou_generator::get_status( DictionaryDatum& d ) const
{
  P_.get( d );
  S_.get( d );
...
inline void
ou_generator::set_status( const DictionaryDatum& d )
{
...
inline SignalType
ou_generator::sends_signal() const
{
  return ALL;
}
}
#endif // OU_GENERATOR_H
