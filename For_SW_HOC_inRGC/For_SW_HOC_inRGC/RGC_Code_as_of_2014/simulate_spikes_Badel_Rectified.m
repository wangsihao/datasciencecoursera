function [Spikes, counter_bins, varargout] = simulate_spikes_Badel_Rectified(total_time,dt,StimParam)
...
%     
    ...
    % RECTIFY FILTER
    %filter_exc = abs(filter_exc);
    %filter_inh = abs(filter_inh);
...
if (StimParam.save_gexc_flag) % && random_flag==2)  
    ...
    if (marg_flag ==1)
        outfname = sprintf('gexc_bin_P_stixel=%d_ID=%d.mat',Stixel_Size,ID_number);
        save(outfname,'stim','g_exc','g_inh','StimParam');
    else
        outfname = sprintf('gexc_gauss_P_stixel=%d_ID=%d.mat',Stixel_Size,ID_number);
        save(outfname,'stim','g_exc','g_inh','StimParam');
    end
end
