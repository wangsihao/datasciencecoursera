/*   reif_cond_alpha_RK5.cpp

     Refractory EIF neuron (w/ alpha synapses)
     A. K. Barreiro 6/8/15
     
     ADAPTED FROM (see below): aeif_cond_alpha_RK5.cpp
 */
...
nest::RecordablesMap<reifnest::reif_cond_alpha_RK5> reifnest::reif_cond_alpha_RK5::recordablesMap_;
...
namespace nest  // template specialization must be placed in namespace
{
  ...
  template <>
  void RecordablesMap<reifnest::reif_cond_alpha_RK5>::create()
  {
    // use standard names whereever you can for consistency!
    insert_(names::V_m, 
	    &reifnest::reif_cond_alpha_RK5::get_y_elem_<reifnest::reif_cond_alpha_RK5::State_::V_M>);
    insert_(names::g_ex, 
	    &reifnest::reif_cond_alpha_RK5::get_y_elem_<reifnest::reif_cond_alpha_RK5::State_::G_EXC>);
    insert_(names::g_in, 
	    &reifnest::reif_cond_alpha_RK5::get_y_elem_<reifnest::reif_cond_alpha_RK5::State_::G_INH>);
    insert_(names::w, 
	    &reifnest::reif_cond_alpha_RK5::get_y_elem_<reifnest::reif_cond_alpha_RK5::State_::W>);
  }
}
...
reifnest::reif_cond_alpha_RK5::Parameters_::Parameters_()
...
reifnest::reif_cond_alpha_RK5::State_::State_(const Parameters_& p)
  : r_(0)
{
  y_[0] = p.E_L;
...
reifnest::reif_cond_alpha_RK5::State_::State_(const State_& s)
  : r_(s.r_)
{
...
reifnest::reif_cond_alpha_RK5::State_& reifnest::reif_cond_alpha_RK5::State_::operator=(const State_& s)
{
...
void reifnest::reif_cond_alpha_RK5::Parameters_::get(DictionaryDatum &d) const
{
...
void reifnest::reif_cond_alpha_RK5::Parameters_::set(const DictionaryDatum& d)
{
  double tmp=0.0;
  ...
  if(updateValue<double>(d,names::MAXERR, tmp))
    {
      if(not (tmp >0.0))
	throw nest::BadProperty("MAXERR must be positive.");
      MAXERR=tmp;
    }
  ...
  if( updateValue<double>(d,names::HMIN, tmp))
    {
      if(not (tmp>0.0))
	throw nest::BadProperty("HMIN must be positive.");
      HMIN=tmp;
    }
  ...
  if ( tau_syn_ex <= 0 || tau_syn_in <= 0 )
    throw nest::BadProperty("All time constants must be strictly positive.");
  
}

void reifnest::reif_cond_alpha_RK5::State_::get(DictionaryDatum &d) const
{
...
void reifnest::reif_cond_alpha_RK5::State_::set(const DictionaryDatum& d, const Parameters_&)
{
...
reifnest::reif_cond_alpha_RK5::Buffers_::Buffers_(reif_cond_alpha_RK5& n)
  : logger_(n)
{
...
reifnest::reif_cond_alpha_RK5::Buffers_::Buffers_(const Buffers_&, reif_cond_alpha_RK5& n)
  : logger_(n)
{
  // Initialization of the remaining members is deferred to
  // init_buffers_().
...
reifnest::reif_cond_alpha_RK5::reif_cond_alpha_RK5()
  : Archiving_Node(), 
    P_(), 
    S_(P_),
    B_(*this)
...
reifnest::reif_cond_alpha_RK5::reif_cond_alpha_RK5(const reif_cond_alpha_RK5& n)
  : Archiving_Node(n), 
    P_(n.P_), 
    S_(n.S_),
    B_(n.B_, *this)
...
void reifnest::reif_cond_alpha_RK5::init_state_(const Node& proto)
{
  const reif_cond_alpha_RK5& pr = downcast<reif_cond_alpha_RK5>(proto);
  S_ = pr.S_;
}

void reifnest::reif_cond_alpha_RK5::init_buffers_()
{
...
void reifnest::reif_cond_alpha_RK5::calibrate()
{
 ...
 * @param to
 */
void reifnest::reif_cond_alpha_RK5::update(Time const & origin, 
					   const nest::long_t from, 
					   const nest::long_t to) // proceed in time
{
  ...
  for ( long_t lag = from; lag < to; ++lag ) // proceed by stepsize B_.step_ 
  {
	...
	do{
	  ...
	  // k1 = f(told, y)
	  reif_cond_alpha_RK5_dynamics(S_.y_,S_.k1);
	  
	  // k2 = f(told + h/5, y + h*k1 / 5)
	  for(int i=0; i < S_.STATE_VEC_SIZE; ++i)
	    S_.yin[i] = S_.y_[i] + h * S_.k1[i]/5.0;
	  reif_cond_alpha_RK5_dynamics(S_.yin,S_.k2);
	  
	  // k3 = f(told + 3/10*h, y + 3/40*h*k1 + 9/40*h*k2)
	  for(int i=0; i < S_.STATE_VEC_SIZE; ++i)
	    S_.yin[i] = S_.y_[i] + h * ( 3.0/40.0*S_.k1[i] 
					 + 9.0/40.0*S_.k2[i] );
	  reif_cond_alpha_RK5_dynamics(S_.yin,S_.k3);
	  
	  // k4
	  for(int i=0; i < S_.STATE_VEC_SIZE; ++i)
	    S_.yin[i] = S_.y_[i] + h * ( 44.0/45.0*S_.k1[i] 
					 - 56.0/15.0*S_.k2[i] 
					 + 32.0/9.0*S_.k3[i] );
	  reif_cond_alpha_RK5_dynamics(S_.yin,S_.k4);
	  
	  // k5
	  for(int i=0; i < S_.STATE_VEC_SIZE; ++i)
	    S_.yin[i] = S_.y_[i] + h * ( 19372.0/6561.0*S_.k1[i] 
					 - 25360.0/2187.0*S_.k2[i] 
					 + 64448.0/6561.0*S_.k3[i] 
					 - 212.0/729.0*S_.k4[i] );
	  reif_cond_alpha_RK5_dynamics(S_.yin,S_.k5);
	  
	  // k6
	  for(int i=0; i < S_.STATE_VEC_SIZE; ++i)
	    S_.yin[i] = S_.y_[i] + h * ( 9017.0/3168.0*S_.k1[i] 
					 - 355.0/33.0*S_.k2[i] 
					 + 46732.0/5247.0*S_.k3[i] 
					 + 49.0/176.0*S_.k4[i] 
					 - 5103.0/18656.0*S_.k5[i] );
	  reif_cond_alpha_RK5_dynamics(S_.yin,S_.k6);
	  
	  // 5th order
	  for(int i=0; i < S_.STATE_VEC_SIZE; ++i)
	    S_.ynew[i] = S_.y_[i] + h * ( 35.0/384.0*S_.k1[i] 
					  + 500.0/1113.0*S_.k3[i] 
					  + 125.0/192.0*S_.k4[i] 
					  - 2187.0/6784.0*S_.k5[i] 
					  + 11.0/84.0*S_.k6[i] );
	  reif_cond_alpha_RK5_dynamics(S_.yin,S_.k7);
	  
	  // 4th order
	  for(int i=0; i < S_.STATE_VEC_SIZE; ++i)
	    {
	      S_.yref[i] = S_.y_[i] + h * ( 5179.0/57600.0*S_.k1[i] 
					    + 7571.0/16695.0*S_.k3[i] 
					    + 393.0/640.0*S_.k4[i] 
...
void reifnest::reif_cond_alpha_RK5::handle(SpikeEvent & e)
{
...
void reifnest::reif_cond_alpha_RK5::handle(CurrentEvent& e)
{
  assert(e.get_delay() > 0);
...
void reifnest::reif_cond_alpha_RK5::handle(DataLoggingRequest& e)
{
  B_.logger_.handle(e);
}
